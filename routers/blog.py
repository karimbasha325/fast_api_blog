...
def create_blog(request: schemas.Blog, db: Session = Depends(get_db)):
    blog = models.Blog(title=request.title, body=request.body, author_id=request.author_id)
    db.add(blog)
    db.commit()
    db.refresh(blog)
    return blog
...
def get_blog(id: int, db:Session = Depends(get_db)):
    ...
    if not blog:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Blog with id {id} is not found in DB")
...
def delete_blog(id: int, db: Session = Depends(get_db)):
    ...
    if not blog.first():
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Blog object with ID {id} is not found in the DB")
...
def update_blog(id: int, request: schemas.Blog, response: Response, db: Session = Depends(get_db)):
    ...
    if not blog.first():
        ...
        return {"detail": "As there is no object available with this ID in the DB, hence created new Blog object with the payload."}
